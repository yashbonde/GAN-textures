...
class Maze(object):
    ...
    def _to_str_matrix(self, _np = False):
        """
        ...
        OOOOOOOOOOO
        """
        # one = int(self.width % 2 == 0)
        one = 1

        if not _np:
            str_matrix = [['O'] * (self.width * 2 + one)
                      for i in range(self.height * 2 + one)]
            ...
            mat = np.zeros((self.height * 2 + one, self.width * 2 + one))
            for cell in self.cells:
                x = cell.x * 2 + one
                y = cell.y * 2 + one
                mat[y][x] = 1
    ...
    @staticmethod
    def generate(width=20, height=10, verbose = False):
        """
        Returns a new random perfect maze with the given sizes.
        """
        m = Maze(width, height)
        m.randomize()
        # print(len(m._to_str_matrix()[0]))
        if verbose:
            for x in m._to_str_matrix():
                print("".join(x))
        return m
...
#                 random.randrange(0, self.maze.height))
    ...
    # def robostep(self, move: int, player):
    #     key = {1: "up", 2: "right", 3: "down", 4: "left"}
    #     direction, difx, dify = {
    #         'up': (N, 0, -1),
    #         'down': (S, 0, 1),
    #         'left': (W, -1, 0),
...
#     Maze.generate().generate(width // 2, height // 2, verbose=True)
